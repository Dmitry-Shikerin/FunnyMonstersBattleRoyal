namespace Fusion.Menu {
  ...
  public partial class FusionMenuUIParty : FusionMenuUIScreen {
    ...
    protected virtual async Task ConnectAsync(bool creating) {
      ...
      if (_regionRequest.IsFaulted || (_regionRequest.IsCompleted && (_regionRequest.Result == null || _regionRequest.Result.Count == 0))) {
        foreach (var exception in _regionRequest.Exception?.Flatten().InnerExceptions) {
          Debug.LogException(exception);
        }
        await Controller.PopupAsync($"Failed to connect and request regions.", "Connection Failed");
        Controller.Show<FusionMenuUIMain>();
        return;
      }
      ...
      if (creating) {
        ...
        if (regionIndex < 0 || regionIndex > _regionRequest.Result.Count) {
