...
{
    public class FlashEase : Ease
    {
        public float Ease(float time, float duration, float overshootOrAmplitude, float period)
        {
        ...
        public override float InOut(float time, float duration, float overshootOrAmplitude, float period)
        {
            ...
            time *= dir;
            float res = (time /= stepDuration * 0.5f) < 1.0
                ? 0.5f * time * time
                : (float)(-0.5 * (--time * (time - 2.0) - 1.0));
