...
namespace Leopotam.EcsProto.Unity {
    ...
    public class ProtoUnityAuthoring : EntityModule 
    {
        [Header("Authoring")]
        ...
        [SerializeField] private bool _authFromFactory = true;
        ...
        {
#if UNITY_EDITOR
            if (Components == null || Components.Count == 0 && _authFromFactory == false)
            {
                throw new Exception ($"[ProtoUnityAuthoring] Пустой список компонентов");
            }
#endif
